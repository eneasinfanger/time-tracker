...
def _parse_activity_time(time_value: str) -> time | None:
    if not time_value:
        return None
    try:
...
def _serialize_client_activity(activity: Activity) -> dict:
    ...
    data['full_name'] = activity.user.full_name if activity.user.full_name else ''
...
def _collect_history_activities(user_id: int, current_date: date, duration_threshold: timedelta,
    ...
    activities = Activity.query.filter(
        ...
        Activity.day_date >= start_bound,
        Activity.day_date < end_bound,
    ).order_by(Activity.day_date.asc(), Activity.position.asc(), Activity.start_time.asc()).all()
...
def get_activities_for_day(activity_date):
    ...
    activities = Activity.query.filter(
        ...
        Activity.day_date == parsed_date,
    ).order_by(Activity.position.asc(), Activity.start_time.asc()).all()
...
def replace_activities_for_day(activity_date):
    ...
    for item in incoming_activities:
        ...
        description = str(item.get('description', '')).strip()
        task = str(item.get('task', '')).strip()
        activity_type = str(item.get('type', '')).strip()

        if activity_type not in ('activity', 'text'):
            continue

        parsed_start_time = _parse_activity_time(start_value)
        parsed_end_time = _parse_activity_time(end_value)

        duration_minutes = 0
        if parsed_start_time and parsed_end_time:
            duration_minutes = _calc_duration_between(parsed_start_time, parsed_end_time).total_seconds() / 60
        ...
        normalized_activities.append({
            ...
            'position': len(normalized_activities),
